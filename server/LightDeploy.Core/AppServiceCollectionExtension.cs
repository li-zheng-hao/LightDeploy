...
using LightDeploy.Core.Authorization;
using LightDeploy.Core.Authorization.Api;
using LightDeploy.Core.Authorization.Hybrid;
using LightDeploy.Core.Authorization.Jwt;
using LightDeploy.Core.Autofac;
using LightDeploy.Core.Converter;
using LightDeploy.Core.FileProvider;
using LightDeploy.Core.Job;
using LightDeploy.Core.Swagger;
using Masuit.Tools;
...
public static class AppServiceCollectionExtension
{
        ...
        WebApplicationBuilder builder)
    {
        serviceCollection.AddInfrastructure(builder.Configuration, configure =>
        {
            ...
            configure.UseMapster(AppDomain.CurrentDomain.GetAssemblies().Where(it=>it.FullName?.StartsWith("LightDeploy")==true).ToArray());
        });

        serviceCollection.AddScoped<UserContext>();
        serviceCollection.AddScoped<IUser,UserContext>();
    ...
    public static IServiceCollection AddApiVersionSetup(this IServiceCollection serviceCollection)
    {
     ...
     public static IServiceCollection AddSwaggerGenSetup(this IServiceCollection serviceCollection,List<string>? documentAssemblyName=null)
    {
        ...
        serviceCollection.AddSwaggerGen(c =>
        {
            ...
            var assemblies =
                documentAssemblyName?.Select(Assembly.Load).ToList();
            if (assemblies != null)
