<template>
    <div class="p-4">
            ...
            <n-space vertical :size="12">
                ...
                <div class="flex flex-wrap items-center gap-4">
                    <div class="flex items-center">
                        ...
                        <n-input v-model:value="searchForm.environment" placeholder="请输入环境" clearable />
                    </div>
                    <div class="flex items-center">
                        <span class="min-w-[80px] text-left mr-2">服务分组：</span>
                        <n-input v-model:value="searchForm.groupName" placeholder="请输入服务分组" clearable />
                    </div>
                    <div class="flex items-center">
                        <span class="min-w-[80px] text-left mr-2">服务名称：</span>
                        <n-input v-model:value="searchForm.serviceName" placeholder="请输入服务名称" clearable />
                    </div>
                    <div class="flex items-center">
        ...
        <!-- 编辑/新增对话框 -->
        <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" :title="modalTitle">
            <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="120"
                ...
                <n-form-item label="环境" path="environment">
                    <n-input v-model:value="formData.environment" placeholder="请输入环境" />
                </n-form-item>
                <n-form-item label="服务分组" path="groupName">
                    <n-input v-model:value="formData.groupName" placeholder="请输入服务分组" />
                </n-form-item>
                <n-form-item label="服务名称" path="serviceName">
                    <n-input v-model:value="formData.serviceName" placeholder="请输入服务名称" />
                </n-form-item>
                <n-form-item label="项目路径" path="projectPath">
                    <n-input v-model:value="formData.projectPath" placeholder="请输入项目路径" />
                </n-form-item>
                <n-form-item label="端口" path="port">
                    <n-input-number v-model:value="formData.port" placeholder="请输入端口号" />
                </n-form-item>
                ...
                <n-form-item label="备注" path="comment">
                    <n-input v-model:value="formData.comment" type="textarea" placeholder="请输入备注" />
                </n-form-item>
                <n-form-item label="仅复制文件" path="onlyCopyFile">
                    <n-checkbox v-model:checked="formData.onlyCopyFile">仅复制文件</n-checkbox>
                </n-form-item>
        ...
        <!-- 发布目标对话框 -->
        <n-modal v-model:show="showTargetModal" :mask-closable="true" style="width: 900px">
            <n-card :title="targetModalTitle">
                ...
                <!-- 目标列表表格 -->
                <n-data-table :columns="targetColumns" :data="targetTableData" :bordered="false" striped />

                <!-- 目标表单对话框 -->
                <n-modal v-model:show="showTargetFormModal" :mask-closable="false" preset="dialog" :title="targetFormTitle">
                    <n-form ref="targetFormRef" :model="targetFormData" :rules="targetRules" label-placement="left" label-width="120"
                        require-mark-placement="right-hanging">
                        <n-form-item label="主机" path="host">
                            <n-input v-model:value="targetFormData.host" placeholder="请输入目标主机" />
                        </n-form-item>
                        <n-form-item label="端口" path="port">
                            <n-input-number v-model:value="targetFormData.port" placeholder="请输入端口号" />
                        </n-form-item>
                        <n-form-item label="服务路径" path="servicePath">
                            <n-input v-model:value="targetFormData.servicePath" placeholder="请输入服务路径" />
                        </n-form-item>
                        <n-form-item label="备注" path="comment">
                            <n-input v-model:value="targetFormData.comment" type="textarea" placeholder="请输入备注" />
                        </n-form-item>
                    </n-form>
                    <template #action>
                        <n-space>
                            <n-button @click="showTargetFormModal = false">取消</n-button>
                            <n-button type="primary" @click="handleSubmitTarget">确定</n-button>
                        </n-space>
                    </template>
                </n-modal>
            </n-card>
...
const filteredData = computed(() => {
    ...
    return tableData.value.filter(item => {
        const environmentMatch = item.environment?.toLowerCase().includes(searchForm.value.environment.toLowerCase())
        const groupNameMatch = item.groupName.toLowerCase().includes(searchForm.value.groupName.toLowerCase())
        const serviceNameMatch = item.serviceName.toLowerCase().includes(searchForm.value.serviceName.toLowerCase())
...
const columns: DataTableColumns<DeployService> = [
    ...
    },
    {
        ...
        key: 'projectType',
        render(row) {
            const option = ProjectTypeOptions.find(opt => opt.value === row.projectType)
